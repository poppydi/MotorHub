...
if (db.prepare('SELECT COUNT(*) AS n FROM faq').get().n === 0) {
  db.prepare(`
    ...
    ('parts', 'Почему один и тот же узел может иметь разные артикулы?', 'Разные производители используют свои каталожные номера. Сверяйте по артикулу, модели авто и кросс-номерам.', 2),
    ...
    ('parts', 'Как понять, что деталь подойдёт к моему автомобилю?', 'Сверьте артикул и описание с вашей модификацией. Если сомневаетесь — уточните у менеджера при оформлении заказа.', 4),
